		...
		function drawLinks(btn_type) {
			...
			/*var appendItem = function(page_id, appendopts, btn_type){
				...
				if(!btn_type && page_id == current_page)
					var lnk = jQuery("<strong>"+(appendopts.text)+"</strong>"); // current
				else if(is_disable)
					var lnk = jQuery("<a>"+(appendopts.text)+"</a>") // <
						.attr('class','pg_prev');
				else {
					var a = jQuery("<a>"+(appendopts.text)+"</a>") // this
						.attr('href', opts.link_to.replace(/__id__/,page_id));
					var lnk = jQuery("<a></a>") // parent <a>
						.bind("click", getClickHandler(page_id, btn_type))
						.attr('class', (btn_type == 'next') ? 'pg_next' : '');
					lnk.append(a);
				}
				panel.append(lnk);
			}*/
			var appendItem = function(page_id, appendopts, btn_type) {
				...
				if (!btn_type && page_id == current_page)
					...
					var lnk = jQuery("<a>" + (appendopts.text) + "</a>") // <
						.attr('class', btn_type == "prev" ? 'pg_prev' : 'pg_next');
				else {
					var a = jQuery("<a>" + (appendopts.text) + "</a>") // this
						.attr('href', opts.link_to.replace(/__id__/, page_id));
					var lnk = jQuery("<a></a>") // parent <a>
						.bind("click", getClickHandler(page_id, btn_type))
						.attr('class', btn_type == "prev" ? 'pg_prev' : (btn_type == "next" ? 'pg_next' : ''));
					lnk.append(a);
				}
				panel.append(lnk);
			}
