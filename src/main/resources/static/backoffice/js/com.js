  ...
  return {
    ...
    callAjax: function (param) {
      ...
      $.ajax({
        ...
        "processData": typeof (param.processData) != "undefined"
    ...
    confirmMsg: function (msg, callback) {
      ...
      if ($("#_comfirmMsg").is(":visible") != true) {
        ...
        // 팝업 표시
        $("#_comfirmMsg").css("display", "flex").hide().fadeIn(500,
            function () {
    ...
    openPopWin: function (formName, url, opt) {
      var winOpt = {
        ...
        "scrollbars": _formLib.getValue(opt.scrollbars) == "" ? "no"
            : opt.scrollbars,
        "resizable": _formLib.getValue(opt.resizeable) == "" ? "no"
            : opt.resizeable,
        "toolbar": _formLib.getValue(opt.toolbar) == "" ? "no" : opt.toolbar,
        "status": _formLib.getValue(opt.status) == "" ? "no" : opt.status
      };

      var popWin = window.open(url, winOpt.name,
          "height=" + winOpt.height + ",width=" + winOpt.width + ",menubar="
          + winOpt.menubar + ",scrollbars=" + winOpt.scrollbars + ",resizable="
          + winOpt.resizable + ",toolbar=" + winOpt.toolbar + ",status="
          + winOpt.status + ",left=" + winOpt.left + ",top=" + winOpt.top);
    ...
    getCookie: function (name) {
      ...
      for (var i = 0; i < cookieList.length; i++) {
        var key = cookieList[i].substr(0, cookieList[i].indexOf("=")).replace(
            /^\s+|\s+$/g, "");
    ...
    preDay: function (cnt) {
      ...
      var date = new Date();
      date.setDate(date.getDate() - cnt); // cnt 일을 빼준다.

      var year = date.getFullYear(); // 빼준 결과에 대한 연도
      var month = date.getMonth() + 1; // 빼준 결과에 대한 월
      var day = date.getDate(); // 빼준 결과에 대한 일
     ...
     * @returns 말일날짜(yyyy-mm-dd)
     */
    prevYear: function (cnt) {
      if (!$.isNumeric(cnt)) {
        cnt = 1;
      }

      var date = new Date();
      var year = new String(date.getFullYear() - cnt);
      var month = new String(date.getMonth() + 1);
      var day = new String(date.getDate());
    ...
    getFileSizeStr: function (bytes) {
      ...
      if (dataChang == "-Infinity") {
        ...
        return (bytes / Math.pow(1024, Math.floor(dataChang))).toFixed(2) + " "
            + dataType[dataChang];
      }
    },
    ...
    print: function (targetId) {
      ...
      var printFrame = $(
          "<iframe name='printFrame' style='position:absolute; top:-1000000px;' />");
      var frameDoc;

      // 인쇄 영역을 바디에 생성
      $("body").append(printFrame);

      frameDoc = printFrame[0].contentWindow ? printFrame[0].contentWindow
          : printFrame[0].contentDocument.document
              ? printFrame[0].contentDocument.document
              : printFrame[0].contentDocument;
      ...
      frameDoc.document.write(
          '<link rel="stylesheet" href="/css/design_custom.css">');
     ...
     * @param targetString
     */
    isValidDate: function (targetString) {
      // 정규식으로 날짜 형식 확인
      var regex = /^\d{4}\.\d{2}\.\d{2} \d{2}:\d{2}$/;
      if (!regex.test(targetString)) return false;

      // Date 객체 생성 및 유효성 검사
      var parts = targetString.split(".");
      var dateParts = parts[2].split(" ");
      var timeParts = dateParts[1].split(":");
      var date = new Date(parts[0], parts[1] - 1, dateParts[0], timeParts[0], timeParts[1]);

      // getMonth는 0부터 시작하므로 1을 더함
      if (date.getFullYear() == parts[0] && date.getMonth() + 1 == parts[1] && date.getDate() == dateParts[0] &&
