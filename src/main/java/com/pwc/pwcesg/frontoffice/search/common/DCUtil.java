...
public class DCUtil {
     ...
     public StringBuffer makeQuery(String nmFd, String kwd, String schMethod, 
		...
		if (query != null && query.length() > 0) {
         ...
         if (kwd != null && kwd.length() > 0) {       
             //if(nmFd.equals("text_idx") || nmFd.equals("cont_nm") || nmFd.equals("vod_comnt") || nmFd.equals("src_nm")) {
            	 
             //}
        	 tempQuery.append(nmFd);
        	 
             if(nmFd.equals("kwrd_arry")) {
	             tempQuery.append(" like '*");
	...
	public StringBuffer makePreQuery(String nmFd, String kwd, String[] prevKwd, 
		...
		//logger.debug("prevKwd : " + prevKwd +" AND prfevKwdLength : "+ prevKwdLength );
		if (prevKwd != null && prevKwdLength > 0) {
			for (int i = 0; i < prevKwdLength; i++) {
				//logger.debug("if 문 true false : " +  escapeQuery(prevKwd[i]).equalsIgnoreCase(kwd));
				//logger.debug(prevKwd[i] + " ? " + kwd);
	...
	/*
	 * public StringBuffer makePreQuery(String nmFd, String kwd, String[] prevKwd,
	 * int prevKwdLength, String schMethod) { StringBuffer query = new
	 * StringBuffer(""); boolean and_ck = true;
	 * 
	 * logger.debug("prevKwd : " + prevKwd + " AND prfevKwdLength : " +
	 * prevKwdLength); if (prevKwd != null && prevKwdLength > 0) { for (int i = 0; i
	 * < prevKwdLength; i++) { logger.debug("if 문 true false : " +
	 * escapeQuery(prevKwd[i]).equalsIgnoreCase(kwd)); logger.debug(prevKwd[i] +
	 * " ? " + kwd);
	 * 
	 * if (!escapeQuery(prevKwd[i]).equalsIgnoreCase(kwd)) { if (and_ck == true) {
	 * query.append(" AND "); and_ck = false; }
	 * 
	 * if (query.length() > 0) { query.append(" AND "); }
	 * 
	 * query.append(nmFd); query.append("='");
	 * query.append(escapeQuery(prevKwd[i]));
	 * //query.append(escapeQuery(CommonUtil.changeEncode(prevKwd[i], "ISO-8859-1",
	 * "EUC-KR"))); //이전 키워드가 깨질 경우 위의 코드로 대체하도록 한다.
	 * 
	 * query.append("' "); query.append(schMethod); } else { if
	 * (escapeQuery(prevKwd[i]).equalsIgnoreCase(kwd)) {
	 * logger.debug("Query append 'AND' error..."); } } } if (query.length() > 0) {
	 * query = new StringBuffer("(").append(query).append(")"); } }
	 * 
	 * return query; }
	 */
	...
	public String getRestURL2(ParameterVO paramVO, SearchVO searchVO) throws UnsupportedEncodingException {
		StringBuffer url = new StringBuffer();
		url.append(searchVO.getUrl());
		url.append("?select=" + searchVO.getFields());
		url.append("&from=" + searchVO.getFrom());
	...
	public String getParamPostData(ParameterVO paramVO, SearchVO searchVO) throws UnsupportedEncodingException {
		...
		System.out.println("getparameterPostData : " + url.toString());
		return url.toString();
	}
	...
	public StringBuffer getUrlDataPost(String urlinfo, String postData, String charset) {
		...
		BufferedReader br = null;
		StringBuffer sb = new StringBuffer();
		System.out.println("DcUtil의 getUrlData 까지 왔음");
		
		// 조회
		try{
			url = new URL( urlinfo);
			System.out.println("getUrlData로 받은 파라미터의 url : " + url);
			conn = (HttpURLConnection) url.openConnection();
