...
import com.pwc.pwcesg.frontoffice.search.data.ResultListVO;
...
public class CommonUtil {
	...
	*/
		...
		for (int i = 0; i < temp.length; i++) {
			if (useIgnoreCase == true) {
				if(temp[i].equalsIgnoreCase(val)) {
					logger.info(">>>>>>>>> equalsIgnoreCase " + val);
					return true;
				}									
			} else {
				if(temp[i].equals(val)) {
					logger.info(">>>>>>>>> equals " + val);
					logger.info("CommonUtil.java --- easyChk End" );
					return true;
				}					
			}			
		}
	 ...
	 * @return boolean
	 */
	public boolean easyChkCategory(String comp, String val, String sepa, boolean useIgnoreCase) {
		if (comp == null || comp.length() == 0 || val == null || val.length() == 0)
			return false;
